...
public class RRScheduler {
    ...
    public static File log = new File("log_RR.txt");                                                        // Creation of File element for logging
    ...
    public static int quantum = 0;                                                                          // The Quantum time in ms

    public static File ss = new File("Snapshot_RR.txt");                                      //Creation of File Element for Snapshot
        ...
        try {
            if (log.exists()==false) {
                ...
                try {
                    ...
                    logger.write(" ");
                    logger.close();*/
                    log = new File("log_RR_"+(a+1)+".txt");                                        //To Change and Try new file name
                    log_creator(a+1);                                                               //Try the nw file name
                }
    ...
    public static void initialize() {                                                   //Initialize needed Processes
        ...
        for (int i = 0; i < initial_values.processes; i++) {
            ...
            ready_queue.add(process_list[i]);                                                               //Adding to Ready Queue
        ...
        for (int i = 0; i < completion_list.length; i++) {
            completion_list[i] = 0;                                                                             //Initializing Completion List
        }
        ...
        try {
            if (ss.exists()==false) {
                ...
                try {
                    ...
                    logger.write(" ");
                    logger.close();*/
                    ss = new File("Snapshot_RR_"+(a+1)+".txt");                                        //To Change and Try new file name
                    ss_creator(a+1);
                }
    ...
    public static void IOgenerator (process p1) {                                           //IO Interrupt generator
        ...
        event e1 = new event(systemClock+t, 1, p1.id);
        addEvent(e1);                                                                                   //Add Interrupt to event list
        logging("\n"+systemClock+": IO Interrupt event generated for time: "+(systemClock+t));          //Log it
    ...
    public static void ioInterrupt(event e1) {                                               //To handle IO Interrupt
        ...
        while (i<t) {                                                                                   //To remove all events related to the process being interrupted
            if (event_List.get(i).process_id == e1.process_id) {
                event_List.remove(i);
                t=event_List.size();
            }
            else{
    ...
    public static void ioCompletion(event e1) {                                                     //To handle IO Completion Event
        ...
        ready_queue.add(process_list[f]);                                                               //Add the process back to ready queue
    ...
    public static void rrInterrupt(event e1) {                              //Handle RR Interrupt
        int f = searchPID(e1.process_id);
        process_list[f].state = 1;                                           //Change Process State             
        ready_queue.add(process_list[f]);                                       //Add it back to the ready queue
        cpu.val = 1;                                                            // Free Up CPU
        cpu.pid = 0;
        ...
        while (i<t) {                                                               //Remove all Events related to the process
            if (event_List.get(i).process_id == e1.process_id) {
                event_List.remove(i);
                t=event_List.size();
            }
    ...
    public static void rrEvent(process p1) {
        event e1 = new event(systemClock+quantum, 3, p1.id);                    //Generate RR Interrupt Event
        addEvent(e1);
    }
   ...
   public static void main(String[] args) throws Exception{
        ...
        /*Scanner reader = new Scanner(System.in);                                         //Object to read in Quantum value
        System.out.print("\nEnter Quantum: ");
        quantum = reader.nextInt();                                                       //Read Quantum Value
        reader.close();*/

        quantum = initial_values.quantum;                                                   //Assign Quantum Value
        ...
        while(flag) {                                                                                               //While loop to go through System Clock
            ...
            if (event_List.size()!=0 && event_List.getFirst().time == systemClock) {                                 //If there is an event with the current System Clock
                while(event_List.size()!=0 && event_List.getFirst().time == systemClock) {                              //To Handle all events with the same Time
                    ...
                    else if (e1.type == 2){                                                                               //IO Completion
                        logging("\n"+systemClock+": IO Completion for Process: "+e1.process_id);
                        ioCompletion(e1);
                    }

                    else if (e1.type == 3) {                                                                                //RR Interrupt
                        logging("\n"+systemClock+": RR Interrupt for Process: "+e1.process_id);
                        rrInterrupt(e1);
                    }
                }
                //tempVal = -1;
            ...
            if (cpu.val == 1 && ready_queue.size()>0) {                                                                         //If CPU is Free and Ready Queue is not empty
                ...
                rrEvent(p1);                                                                                                    //Generate RR Interrupt for the process
                tempVal = pi;
